...
Carte interactive Tadoussac animaux marins

GPT-4

User
have a streamlit webapp. Create a subpage named: discover. In the page there is a map containing 3 dots near tadoussac in the water. If you click on the dot, there is a description about a marine animal located there. Make an script and add the descriptiopn for whale, seal, beluga

ChatGPT
Certainly! To create a Streamlit subpage with an interactive map displaying dots near Tadoussac and showing descriptions of marine animals when clicked, you can utilize the pydeck package for map rendering along with Streamlit's native functionality.
...
def main():
    st.title("Marine Animal Discoverer")
    st.sidebar.title("Navigation")
    page = st.sidebar.selectbox("Choose a page", ["Home", "Discover"])

    if page == "Discover":
        display_animal_map()
    elif page == "Home":
        st.write("Welcome to the Marine Animal Discoverer!")

if __name__ == "__main__":
    main()
Note: This code assumes you have streamlit and pydeck packages installed. If you don't, you can install them using pip:

Copy code
pip install streamlit pydeck
After running the above code with streamlit run script_name.py, it will display a subpage with a map and three dots representing the animals. When a dot is clicked, the corresponding animal's description will be shown.




User
Clicking on the dot doesnt work

ChatGPT
I apologize for the oversight. Streamlit and pydeck integration can sometimes be a bit tricky.

Let's modify the code to display the description when a point on the map is clicked:
